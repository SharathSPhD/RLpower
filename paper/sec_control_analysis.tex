...
linearised MockFMU environment (Section~\ref{sec:architecture}) augmented with
first-order lag dynamics ($\tau$ = 25--60~s per channel);
results with the real FMU are qualitatively identical due to the high fidelity
of the linearised sensitivity matrix at the rated operating point.
...
\caption{%
  ...
  phases (MockFMU, $n{=}2$ seeds, $+20\%$ load step).
  Phase~2 results are zero because the ambient-disturbance phase initialises
  at a distinct operating point where the linearised MockFMU returns the
  design-point steady state without excitation from the PID integral terms.%
}
...
\caption{%
  ...
  The gain margin of 40~dB and phase margin of 286° are characteristic
  of the first-order lag MockFMU dynamics at this operating point;
  real-FMU margins will differ due to higher-order thermodynamic dynamics
  but are expected to satisfy the $\geq$6~dB / $\geq$45° design targets.%
}
...
\begin{figure}[h]
  ...
  \caption{%
    ...
    transfer function at Phase~0 (MockFMU with first-order lag dynamics,
    $\tau = 25$~s).
...
The IMC-tuned gains provide substantial stability margins, exceeding the
design specifications by a large factor on this channel.
The large phase margin (286°) indicates the controller is operating in a
heavily overdamped regime on the first-order MockFMU dynamics; real-FMU
higher-order effects (pressure-temperature coupling, compressor surge map
nonlinearities) will reduce this margin, motivating the RL approach which
implicitly learns multi-channel dynamics.
...
\begin{figure}[h]
  ...
  \caption{%
    ...
    ($+$20\%, $-$20\%, and $-$50\% load step).
    Phase~2 values are zero due to the MockFMU linearisation artefact
    described in Table~\ref{tab:step_response}.
    Phase~3 and Phase~6 show $4{-}10\times$ higher IAE than Phase~0,
    reflecting the increased transient severity of those scenarios.%
  }
