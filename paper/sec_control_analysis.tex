...
\centering
\caption{%
  Frequency-domain stability margins for PID and RL controllers,
  bypass-valve $\to$ $W_{\mathrm{net}}$ channel, Phase~0.
  The gain margin and phase margin are estimated from PRBS excitation
  on the MLP surrogate.
  Both controllers satisfy the $\geq$6~dB / $\geq$45° design targets.%
}
\label{tab:stability_margins}
\begin{tabular}{lccc}
\toprule
Metric & PID (IMC-tuned) & RL (PPO-MLP) & Target \\
\midrule
Gain Margin (dB) & 40.0 & 43.0 & $\geq 6$ \\
Phase Margin (deg) & 285.9 & 301.2 & $\geq 45$ \\
Bandwidth (Hz) & 0.00625 & 0.0075 & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h]
  ...
  \caption{%
    ...
    at Phase~0 (MLP surrogate, PRBS excitation, Welch cross-spectrum).
    PID (blue solid) and RL (red dashed) gain and phase responses.
    The RL controller achieves 3~dB higher gain margin and 15\textdegree{}
    additional phase margin relative to the PID baseline.%
  }
