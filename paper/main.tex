...
curriculum phases within 229{,}376 steps, reaching Phase~6 (emergency turbine
trip recovery) with a mean episode reward of \textbf{399.9} and a constraint
violation rate of \textbf{0.000}.
At step 212{,}992 with all bugs fixed, the RL controller achieves a
\textbf{+17.6\%} improvement in cumulative episode reward over a PID baseline
(806.6 vs.\ 685.9).
...
\begin{table}[h]
\centering
\caption{Curriculum advancement timeline (corrected training run).}
...
\midrule
\multicolumn{2}{l}{Ongoing (Phase 6)} & 399.9 & 0.000 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Phase 0: Steady-State Performance}

\begin{table}[h]
\centering
\caption{Phase~0 steady-state controller comparison (10-episode evaluation
         at step~212{,}992).}
\label{tab:phase0}
\begin{tabular}{lcc}
\toprule
Metric & RL (PPO + Lagrangian) & PID Baseline \\
\midrule
Mean raw episode reward & $806.6$ & $685.9$ \\
Improvement over PID & \multicolumn{2}{c}{$+17.6\%$} \\
Constraint violation rate & $0.000$ & $0.000$ \\
\bottomrule
\end{tabular}
\end{table}

Both controllers satisfy all physical safety constraints.
The RL policy achieves higher tracking accuracy by learning to pre-position
valves ahead of small steady-state perturbations, exploiting the asymmetric
gain of the near-critical CO$_2$ thermodynamics in a way a fixed-gain PID
cannot.

\subsection{Transient Phases 1--6}

Following full curriculum traversal, the agent operates in Phase~6 (emergency
turbine trip recovery) with a mean episode reward of 399.9 and zero constraint
violations, demonstrating successful generalisation to the hardest training
scenario.
The reward trend is monotonically increasing throughout Phase~6 training
(373.4 $\to$ 379.4 $\to$ 399.9 over successive checkpoints), indicating
continued policy improvement.

\begin{table}[h]
\centering
\caption{Phase~6 (emergency turbine trip recovery) metrics during ongoing training.}
\label{tab:phase6}
\begin{tabular}{lcc}
\toprule
Metric & Value & Notes \\
\midrule
Current training step & 802{,}816 & of 5{,}000{,}000 total \\
Mean episode reward & $399.9$ & increasing \\
Constraint violation rate & $0.000$ & zero violations \\
Throughput & ${\approx}534$~steps/s & 8 CPU FMU workers \\
ETA to completion & ${\approx}131$~min & at current throughput \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  \item Successfully traverses all 7 curriculum phases (Phases 0--6) within
        229{,}376 training steps once infrastructure bugs are resolved.
  \item Achieves a +17.6\% improvement in episodic reward over a PID baseline
        in steady-state conditions (806.6 vs.\ 685.9).
  \item Operates in Phase~6 (emergency turbine trip recovery — the hardest
        curriculum scenario) with mean reward 399.9 and zero constraint
        violations.
  \item Satisfies deployment latency requirements with p99 = 0.046~ms
        ($22\times$ margin against the 1~ms SLA).
\end{itemize}
...
\begin{enumerate}
  \item \textbf{Complete 5M-step training run.}
        The ongoing FMU training run (5M total steps, ${\approx}131$~min
        remaining) will provide convergence curves and final per-phase
        evaluation across all 7 curriculum scenarios.
  \item \textbf{Surrogate dataset collection and retraining.}
        ...
        reducing training time by ${\sim}2000\times$.
